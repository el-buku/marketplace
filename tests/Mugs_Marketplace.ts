...
describe('Mugs_Marketplace Load Program Object & Prepare testers', () => {
  ...
  console.log('MARKETPLACE_PROGRAM_ID:', MARKETPLACE_PROGRAM_ID.toBase58());
  ...
  it('Load Reward Token', async () => {
    ...
    console.log('Reward Token Mint: ', reward.publicKey.toBase58());
...
describe('Contract Creation', async () => {
  it('Contract creator has a role of Admin', async () => {
    const tx = await createInitializeTx(superOwner.publicKey, program as unknown as anchor.Program);
  ...
  it('Admin can change the Marketplace Fee', async () => {
    ...
    const tx = await createUpdateFeeTx(superOwner.publicKey, program as unknown as anchor.Program, 100, 12);
    const txId = await provider.connection.sendTransaction(tx, [superOwner]);
    await provider.connection.confirmTransaction(txId, 'confirmed');
    console.log('TxHash=', txId);
...
describe('NFT Listing / Cancel Listing', async () => {
  ...
  it('User can init NFT SellData PDA', async () => {
    ...
    const tx = await createInitSellDataTx(nft.publicKey, user.publicKey, program as unknown as anchor.Program);
  ...
  it('Mint one NFT in my ATA for testing', async () => {
    const userNFTAccount = await nft.createAssociatedTokenAccount(user.publicKey);
...
describe('user1 Can Purchase NFT', async () => {
  it('Mint Enough ABB TOken in user1 ATA for staking', async () => {
    const rewardToken = new Token(provider.connection, ABB_TOKEN_MINT, TOKEN_PROGRAM_ID, superOwner);
    ...
    if (instructions.length > 0) {
      ...
      const txId = await anchor.web3.sendAndConfirmTransaction(provider.connection, tx, [user1]);
  ...
  it('User can init own UserPool PDA', async () => {
    ...
    const tx = await createInitUserTx(user.publicKey, program as unknown as anchor.Program);
    const txId = await provider.connection.sendTransaction(tx, [user]);
    await provider.connection.confirmTransaction(txId, 'confirmed');
    console.log('TxHash=', txId);
  ...
  it('User1 can init own UserPool PDA', async () => {
    ...
    const tx = await createInitUserTx(user1.publicKey, program as unknown as anchor.Program);
  ...
  it('user1 can purchase NFT from user with ABB Token payment', async () => {
    ...
    assert(
      buyerUserInfo.tradedTokenVolume.toNumber() == nftInfo.priceToken.toNumber() - fee_amount,
    ...
    assert(
      sellerUserInfo.tradedTokenVolume.toNumber() == nftInfo.priceToken.toNumber() - fee_amount,
      'UserData TradeVolume is not priceToken',
    );
  });
  return;
